            ...
            async with ctx.typing():
                ...
                for j in range(5):
                    ...
                    def map():
                        try:
                            return str(
                                jsonR['data']['matches'][j]['metadata']['map'])
                        except KeyError:
                    ...
                    for i in players:
                        if i['name'] == name or i['name'] == name.title():
                            ...
                            embedM.add_field(
                                name='--------------------------',
                                value=f"**{mode()} | {map()}** | ***{won}-{lost}***",
                                inline=False)
                            embedM.add_field(
                                name='Character: ', value=i['character'])
                            embedM.add_field(name='KDA: ', value=str(i['stats']['kills']) + '/' + str(
                                i['stats']['deaths']) + '/' + str(i['stats']['assists']), inline=True)
                            embedM.add_field(
                                name='Combat Score: ',
                                value=int(
                                    i['stats']['score']) // int(
